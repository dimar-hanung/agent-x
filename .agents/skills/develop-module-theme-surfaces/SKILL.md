...
- Kanban columns in `components/dashboard/todos/todo-kanban-column.tsx` use `bg-card` on the recessed well so columns read as raised containers; todo cards still pop via `shadow-xs` and border on the same fill.
