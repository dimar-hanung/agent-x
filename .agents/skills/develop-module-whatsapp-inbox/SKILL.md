...
- Settings UI: `components/settings/whatsapp-inbox-connect-card.tsx`
- Dashboard: `app/dashboard/whatsapp-inbox/` — snapshot-centric UI (`snapshot-list`, `snapshot-panel`)
...
- Global channel webhook path unchanged (AI auto-reply to registered phones).
...
- **Dashboard:** snapshot-first — list recent snapshots, view all-chat digest body; no per-DM/per-group summary panel.
