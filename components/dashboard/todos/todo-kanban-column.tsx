...
export function TodoKanbanColumn({
  ...
  if (collapsed) {
    return (
      <div
        ref={setNodeRef}
        className={cn(
          "bg-card flex min-h-[320px] w-11 shrink-0 flex-col items-center rounded-lg border transition-[border-color,box-shadow] duration-200 ease-out",
  ...
  return (
    <div
      className={cn(
        "bg-card flex min-h-[320px] min-w-0 flex-1 flex-col rounded-lg border transition-[border-color,box-shadow] duration-200 ease-out",
        isHighlighted && "border-ring ring-ring ring-1 ring-inset"
      )}
    >
      <button
