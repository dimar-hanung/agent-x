...
import type { FileIndexProgressPhase } from "@/lib/files/constants";
...
export async function updateFileIndexProgress(
  fileId: string,
  progress: {
    phase: FileIndexProgressPhase;
    current?: number | null;
    total?: number | null;
  }
): Promise<void> {
...
export async function getFileIndexStatus(
  ...
  const [row] = await db
    .select({
      ...
      progressPhase: userFileIndexes.progressPhase,
      progressCurrent: userFileIndexes.progressCurrent,
      progressTotal: userFileIndexes.progressTotal,
    })
