...
export async function runIndexJobForFile(
  ...
  try {
    await updateFileIndexProgress(fileId, { phase: "downloading" });
    const { body: buffer } = await getObjectBytes(row.storageKey);

    await updateFileIndexProgress(fileId, { phase: "chunking" });
    const [chunkResponse, previewMarkdown] = await Promise.all([
      chunkHybridFile({
        buffer,
        filename: row.name,
        onPollHeartbeat: () =>
          updateFileIndexProgress(fileId, { phase: "chunking" }),
      }),
    ...
    const embeddings = await embedTexts(texts, {
      onBatch: ({ current, total }) =>
        updateFileIndexProgress(fileId, {
          phase: "embedding",
          current,
          total,
        }),
    ...
    await updateFileIndexProgress(fileId, { phase: "saving" });
