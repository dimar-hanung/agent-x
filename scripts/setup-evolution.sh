...
echo "==> Waiting for Evolution API on :8080..."
for i in $(seq 1 30); do
  if curl -sf -o /dev/null -H "apikey: $(grep '^AUTHENTICATION_API_KEY=' .env | cut -d= -f2-)" http://127.0.0.1:8080/ 2>/dev/null; then
...
echo "Stack started. Check logs: cd infra/evolution && docker compose logs -f evolution-api"
exit 0
